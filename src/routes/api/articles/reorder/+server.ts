...
/**
 * Article 목록의 priority를 재정렬합니다 (범위가 너무 좁아진 경우 사용)
 */
...
export const POST = async ({ request, platform }) => {
	...
	const { articles } = (await request.json()) as { articles: string[] };
	...
	try {
		...
		const allArticles = await db
			.select()
			.from(article)
			.orderBy((a) => [a.priority, a.createdAt]);

		// ID를 기준으로 기존 priority 맵 생성
		const prevPriorityMap = new Map(allArticles.map((a) => [a.id, a.priority]));
		...
		for (let i = 0; i < articles.length; i++) {
			...
			// 이전 위치의 priority
			const prevPosition =
				i > 0 ? (updates[i - 1]?.priority ?? prevPriorityMap.get(articles[i - 1]) ?? null) : null;

			// 다음 위치의 priority
			const nextPosition =
				i < articles.length - 1 ? (prevPriorityMap.get(articles[i + 1]) ?? null) : null;
		...
		// 범위가 너무 좁으면 전체 재정렬
		if (needsReset) {
			const resetPrioritiesList = resetPriorities(articles.length);
			for (let i = 0; i < articles.length; i++) {
				const id = articles[i];
				const prevPriority = prevPriorityMap.get(id);
		...
		for (const { id, priority } of updates) {
			await db.update(article).set({ priority, updatedAt: new Date() }).where(eq(article.id, id));
		}
	} catch (err) {
		console.error('Article reorder error:', err);
		return error(500, 'Failed to reorder articles');
	}
