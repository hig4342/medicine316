...
allowed-tools: Bash(pnpx shadcn-svelte@latest *), Bash(pnpm dlx shadcn-svelte@latest *), Bash(bunx --bun shadcn-svelte@latest *)
---
...
> **IMPORTANT:** Run all CLI commands using the project's package runner: `pnpx shadcn-svelte@latest`, `pnpm dlx shadcn-svelte@latest`, or `bunx --bun shadcn-svelte@latest` — based on the project's package manager. Examples below use `pnpx shadcn-svelte@latest` but substitute the correct runner for the project.
...
1. **Use existing components first.** Run `pnpx shadcn-svelte@latest add` with no arguments to browse available components, or check [Components](https://shadcn-svelte.com/docs/components) before writing custom UI.
...
- **Presets** — copy the encoded string from the design-system builder on [shadcn-svelte.com](https://shadcn-svelte.com) and pass it to `pnpx shadcn-svelte@latest init --preset <code>`.
...
3. **Discover components** — `pnpx shadcn-svelte@latest add` with no arguments (interactive list), or the docs site.
4. **Install or update** — `pnpx shadcn-svelte@latest add <name>` or a registry **URL**. To refresh existing files from the registry, use `pnpx shadcn-svelte@latest update` (see [cli.md](./cli.md)).
...
2. Run `pnpx shadcn-svelte@latest update [component]` or `--all`.
...
pnpx shadcn-svelte@latest init --preset <code>

# Add components (interactive when run with no names).
pnpx shadcn-svelte@latest add
pnpx shadcn-svelte@latest add button card dialog
pnpx shadcn-svelte@latest add --all

# Update components already installed.
pnpx shadcn-svelte@latest update button
pnpx shadcn-svelte@latest update --all --yes

# Build a custom registry (registry authors).
pnpx shadcn-svelte@latest registry build
```
